...
{
  /**
export async function getSingleSnacks(id: string) {
  var doc = await firestore.collection("snacks").doc(id).get();
  return doc;
}
export async function getSingleMenu(chefId: string, memuId: string) {
...
export async function getTotalPrice(data) {
  let total: number = 0;
  console.log(data);

  for (let i = 0; i < data.length; i++) {
    let multiplyPriceByQuantity: number = 0;

    console.log(data[i].type);
    if (data[i].type === "snack") {
      const doc = await getSingleSnacks(data[i].menu.id);
      console.log(
        "snack: ",
        doc.data().price,
        data[i].menu.quantity,
        (doc.data().price * data[i].menu.quantity).toFixed(2)
      );
      multiplyPriceByQuantity = parseFloat(
        (doc.data().price * data[i].menu.quantity).toFixed(2)
      );
    } else {
      const doc = await getSingleMenu(data[i].chef.id, data[i].menu.id);
      console.log(
        "menu: ",
        doc.data().price,
        data[i].menu.quantity,
